...
class PublisherBuilder[U, V]:
    ...
    def add_serializer[U2](self, serializer: Serializer[U2, U] | None) -> PublisherBuilder[U2, V]:
        ...
        def wrapper(inner: RawPublisher) -> Publisher[U2, V]:
            return EncodingPublisher(inner_wrapper(inner), serializer.dump_message)
...
class ConsumerBuilder[U, V]:
    ...
    def add_serializer[U2](self, serializer: Serializer[U2, U] | None) -> ConsumerBuilder[U2, V]:
        if serializer is None:
        ...
        def wrapper(inner: Consumer[U2, V]) -> RawConsumer:
            return inner_wrapper(DecodingConsumer(inner, serializer.load_message))
        ...
        match inner:
            ...
            case func:
                if inspect.iscoroutinefunction(func):
                    consumer = AsyncFuncConsumer(func)

                else:
                    sync_func = t.cast(t.Callable[[U], V], func)
                    consumer = SyncFuncConsumer(sync_func, executor)
